...
import (
	...
	"github.com/damillano93/usuarios/models"
...
func (c *UsuariosController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		fmt.Println(hash(v.Password))
		var cifrado = hash(v.Password)
		v.Password = fmt.Sprint(cifrado)
		if _, err := models.AddUsuarios(&v); err == nil {
			c.Ctx.Output.SetStatus(201)
			c.Data["json"] = v
		} else {
			alert := models.Alert{Type: "error", Code: "E_400", Body: err.Error()}
			c.Data["json"] = alert
		}
	} else {
		alert := models.Alert{Type: "error", Code: "E_400", Body: err.Error()}
...
func (c *UsuariosController) GetAll() {
	...
	if v := c.GetString("query"); v != "" {
		for _, cond := range strings.Split(v, ",") {
			...
			if len(kv) != 2 {
				alert := models.Alert{Type: "error", Code: "E_400", Body: "Error: invalid query key/value pair"}
				c.Data["json"] = alert
...
func (c *UsuariosController) Put() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateUsuariosById(&v); err == nil {
			alert := models.Alert{Type: "success", Code: "200", Body: "OK"}
			c.Data["json"] = alert
		} else {
			alert := models.Alert{Type: "error", Code: "E_400", Body: err.Error()}
			c.Data["json"] = alert
